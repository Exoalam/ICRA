...
# cv2.waitKey(0)
# cv2.destroyAllWindows()
xd = round(float(input()),2)
yd = round(float(input()),2)
zd = round(float(input()),2)
with open('vs3.csv', 'a', newline='') as file:
    writer = csv.writer(file)
    writer.writerow([np.round(xx,2),np.round(yy, 2), np.round(zz, 2),xd, yd, zd,np.round(np.sqrt(pow(xx-xd,2)+pow(yy-yd,2)+pow(zz-zd,2)),2)])
